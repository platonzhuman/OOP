...
void iosev()
{
    ...
    cin >> k;
    if (k <= 0) {
        cout << "k должно быть положительным, установлено k = 2" << endl;
        k = 2;
    }
    ...
    // открываем файл для автоматической записи результатов в grafik.py чтобы в последующем легче было проверять результаты
    ofstream res_file("result.csv");
    res_file << "k," << k << "\n";
    ...
    for (int i = 0; i < num_tests; ++i)
    {
        ...
        // Сохраняем n и время в наш файл
        res_file << n << "," << elapsed.count() << "\n";
    }
    res_file.close();
    cout << "кезультаты сохранены в results.csv\n";
}
